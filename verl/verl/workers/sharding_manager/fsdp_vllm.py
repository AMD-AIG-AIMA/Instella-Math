...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def update_params(self, updated_params):
        ...
        if model.config.architectures[0] == 'InstellaForCausalLM':
            # Use the new patched function for Instella
            param_list = []
            for name, param in updated_params.items():
                processed_param = param.full_tensor() if world_size != 1 and hasattr(param, 'full_tensor') else param
                param_list.append((name, processed_param))
            
            loaded_params = patched_instella_load_weights(model, param_list)
        else:
...
# https://github.com/volcengine/verl/blob/main/verl/workers/sharding_manager/patch/fsdp_vllm_patch.py#L24
# based on teh above patch
def patched_instella_load_weights(model, weights):
    """
    Custom weight loading function for Instella model.
    
    Args:
        model: The model to load weights into
        weights: An iterable of (name, weight) tuples
        
    Returns:
        A set of loaded parameter names
    """
    import re
    import torch
    ...
    # Define parameter mappings similar to the ones in the DeepseekV3 function
    ...
    logger.info(f"Model parameters count: {len(params_dict)}")
    
    # Convert to list to avoid consuming the iterator
    weight_list = list(weights)
    logger.info(f"Input weights count: {len(weight_list)}")
    
    for name, loaded_weight in weight_list:
        if "rotary_emb.inv_freq" in name:
            continue
            
        # Check for stacked parameters first
        matched = False
        for (param_name, weight_name, shard_id) in stacked_params_mapping:
            ...
            param_full_name = name.replace(weight_name, param_name)
            if param_full_name.endswith(".bias") and param_full_name not in params_dict:
                continue
                
            # Skip if parameter is missing due to pipeline parallelism
            if is_pp_missing_parameter(param_full_name, model):
                continue
                
            # Get the parameter and its weight loader
            if param_full_name in params_dict:
        ...
        if not matched:
            ...
            # Handle KV scale remapping if needed
            remapped_name = maybe_remap_kv_scale_name(name, params_dict)
            if remapped_name is None:
                # Check if the parameter exists in the model without remapping
            ...
            # Skip if parameter is missing due to pipeline parallelism
            if is_pp_missing_parameter(remapped_name, model):
                continue
    ...
    # Print loaded parameters count for debugging
    logger.info(f"Loaded parameters count: {len(loaded_params)}")
