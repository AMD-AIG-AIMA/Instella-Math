...
from deepscaler.globals import THOUGHT_DELIMITER_START, THOUGHT_DELIMITER_END, OAI_RM_MODEL, THOUGHT_DELIMITER_END_exaone, THOUGHT_DELIMITER_END_still
...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        if "Llama-3.2-3B-Instruct" in model_path.split("/")[-1]:
            model_solution = model_response
        elif "qwen2-math" in model_path.split("/")[-1].lower() or "limo" in model_path.split("/")[-1].lower() or "sky-t1" in model_path.split("/")[-1].lower() or "openthinker-" in model_path.split("/")[-1].lower() or "qwq-" in model_path.split("/")[-1].lower():
            model_solution = model_response
        elif "s1.1-" in model_path.split("/")[-1].lower():
            model_solution = model_response
        else:
            if "exaone" in model_path.split("/")[-1].lower() or "s1" in model_path.split("/")[-1].lower():
                model_solution = model_response #.split(THOUGHT_DELIMITER_END_exaone)[1]
...
def deepscaler_reward_fn(model_path: str, solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False):
    reward_config = RewardConfig()
    reward_config.use_math_orm = enable_llm
    reward_fn = RewardMathFn(reward_config)
    reward_response = reward_fn(RewardInput(model_path=model_path, problem=solution_str, problem_type=RewardType.MATH, model_response=solution_str, ground_truth={"answer": ground_truth}))
